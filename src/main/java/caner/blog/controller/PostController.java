...
public class PostController {
    ...
    public String updatePost(@Valid @ModelAttribute("post") UpdatePostRequest updatePostRequest,
        ...
        if (result.hasErrors()) {
            if (result.getFieldErrors().stream().map(FieldError::getField).toList().contains("title")) {
                redirectAttributes.addFlashAttribute("titleError", "Başlık 5-30 karakter arasında olmalıdır!");
                return "redirect:/posts/update-post/" + id;
            }

            if (result.getFieldErrors().stream().map(FieldError::getField).toList().contains("shortInformation")) {
                redirectAttributes.addFlashAttribute("shortInformationError", "Kısa bilgi 5-50 karakter arasında olmalıdır!");
                return "redirect:/posts/update-post/" + id;
            }

            if (result.getFieldErrors().stream().map(FieldError::getField).toList().contains("content")) {
                redirectAttributes.addFlashAttribute("contentError", "İçerik boş olamaz!");
                return "redirect:/posts/update-post/" + id;
            }
        }

        Post post = postService.updatePost(updatePostRequest, request);
